...
vector<vector<pair<int, int>>> create_edge_sets(const vector<pair<int, int>> &edges) {
    vector<vector<pair<int, int>>> edge_sets;
    for (const auto &edge : edges) {
        bool added = false;
        for (auto &edge_set : edge_sets) {
            bool conflict = false;
            for (const auto &existing_edge : edge_set) {
                if (edge.first == existing_edge.first || edge.first == existing_edge.second ||
                    edge.second == existing_edge.first || edge.second == existing_edge.second) {
                    conflict = true;
                    break;
                }
            }
...
void simulate(float h, const MatrixXf &rest_vertices, const MatrixXi &faces, const vector<pair<int,int>> &edges,
    ...
    vector<vector<pair<int, int>>> edge_sets = create_edge_sets(edges);
    int chromatic_index = edge_sets.size();
    ...
    for(int i=0;i<total_frames;++i){
        ...
        for(int j=0;j<chromatic_index;++j){
            x = gpbd_step(h, x, masses, rest_vertices, edge_sets[j], constraint_vertices, gpbd_iters, stiffness, i);
        }
